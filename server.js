...
app.get('/health', (req, res) => {
  res.status(200).json({ status: 'ok', service: 'CyberReport' });
});
...
async function startServer() {
  ...
  console.log(`║           ${config.APP_BASE_URL || `http://localhost:${config.PORT}`}                  ║`);
  console.log('║                                                            ║');
  console.log(`║      Admin : ${(config.APP_BASE_URL || `http://localhost:${config.PORT}`)}/admin/login   ║`);
...
startServer().catch((error) => {
  console.error('❌ Impossible de démarrer CyberReport :', error);
  process.exit(1);
});

// config/mailer.js – Service d'envoi d'emails avec Resend uniquement
...
async function verifyMailerConnection() {
  console.log('Service email sélectionné : Resend');

  if (!hasEmailConfig()) {
    console.warn('Emails désactivés : RESEND_API_KEY ou EMAIL_FROM manquant.');
    return false;
  }
...
async function sendMail(options) {
  if (!hasEmailConfig()) {
    console.warn(`Email non envoyé à ${options.to || 'destinataire inconnu'} : Resend non configuré.`);
    return false;
  }
  ...
  if (options.reply_to || config.EMAIL.replyTo) payload.reply_to = options.reply_to || config.EMAIL.replyTo;
...
async function sendCommentEmail(incident, comment, adminName = 'Administrateur') {
  const html = renderLayout({
    eyebrow: 'Message de l’administration',
    title: `Nouveau commentaire sur votre ticket ${incident.ticket_number}`,
    subtitle: 'Un membre de l’équipe vient d’ajouter une information à votre signalement CyberReport.',
    accent: '#6366f1',
    bodyHtml: `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.8;">Bonjour <strong>${escapeHtml(incident.prenom)} ${escapeHtml(incident.nom)}</strong>,</p>
      <p style="margin:0 0 18px;font-size:15px;line-height:1.8;">Un commentaire a été ajouté par l’administration concernant votre incident.</p>

      <div style="padding:18px 20px;border-radius:18px;background:#f8fafc;border:1px solid #e2e8f0;margin-bottom:18px;">
        <div style="font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#64748b;margin-bottom:10px;">Commentaire envoyé</div>
        <div style="font-size:15px;line-height:1.8;color:#0f172a;white-space:pre-wrap;">${escapeHtml(comment.commentaire || comment)}</div>
        <div style="margin-top:12px;font-size:12px;color:#64748b;">Envoyé par : ${escapeHtml(adminName || 'Administrateur')} · ${formatDate(comment.date || new Date())}</div>
      </div>

      <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;margin:0 0 18px;">
        ${renderInfoRows([
          ['Ticket', incident.ticket_number],
          ['Titre', incident.titre_incident || 'Incident'],
          ['Statut actuel', getStatusLabel(incident.statut)],
          ['Gravité', incident.gravite],
          ['Dernière mise à jour', formatDate(incident.date_modification)]
        ])}
      </table>

      <a href="${PUBLIC_TRACKING_URL}" style="display:inline-block;background:linear-gradient(135deg,#6366f1,#0ea5e9);color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:14px;font-weight:800;">
        Consulter le suivi du ticket
      </a>
    `,
    footer: 'CyberReport · Message automatique envoyé depuis l’espace administration'
  });
  ...
  try {
    await sendMail({
      from: config.EMAIL.from,
      to: incident.email,
      subject: `Nouveau commentaire – Ticket ${incident.ticket_number}`,
      html
    });
    console.log(`Commentaire envoyé par email à ${incident.email}`);
    return true;
  } catch (err) {
